use crate::AppConfig;
use anyhow::{Context, Result, bail};
use sea_query::{
    ColumnDef, Expr, ForeignKey, Iden, Index, MysqlQueryBuilder, PostgresQueryBuilder,
    QueryStatementWriter, SchemaStatementBuilder, SqliteQueryBuilder, Table, Values,
};
use sea_query_sqlx::SqlxValues;
use sqlx::AnyConnection;
use sqlx::Connection;
use sqlx::any::install_default_drivers;
use tracing::{debug, info};

pub struct DBConnection {
    pub connection: AnyConnection,
    pub wrap_timestamp: bool,
    pub backend: Backend,
}

#[derive(Copy, Clone, Debug, PartialEq, Eq)]
pub enum Backend {
    Postgres,
    MySql,
    Sqlite,
}

impl Backend {
    pub fn build_query<Q: QueryStatementWriter>(
        self,
        stmt: &Q,
    ) -> (sqlx::AssertSqlSafe<String>, SqlxValues) {
        let (sql, values): (String, Values) = match self {
            Backend::Postgres => stmt.build(PostgresQueryBuilder),
            Backend::MySql => stmt.build(MysqlQueryBuilder),
            Backend::Sqlite => stmt.build(SqliteQueryBuilder),
        };
        (sqlx::AssertSqlSafe(sql), SqlxValues(values))
    }

    pub fn build_schema<S: SchemaStatementBuilder>(self, stmt: &S) -> String {
        match self {
            Backend::Postgres => stmt.build(PostgresQueryBuilder),
            Backend::MySql => stmt.build(MysqlQueryBuilder),
            Backend::Sqlite => stmt.build(SqliteQueryBuilder),
        }
    }
}

pub enum EmailRoute {
    Table,
    Id,
    Domain,
    User,
    Url,
    SecretToken,
    IsEnabled,
}
impl Iden for EmailRoute {
    fn unquoted(&self) -> &str {
        match self {
            Self::Table => "email_routes",
            Self::Id => "id",
            Self::Domain => "domain",
            Self::User => "user",
            Self::Url => "url",
            Self::SecretToken => "secret_token",
            Self::IsEnabled => "is_enabled",
        }
    }
}

pub enum WebhookQueue {
    Table,
    Id,
    EmailRouteId,
    Payload,
    Attempts,
    NextRetryAt,
    LastError,
    IsExpired,
    CreatedAt,
}
impl Iden for WebhookQueue {
    fn unquoted(&self) -> &str {
        match self {
            Self::Table => "webhook_queue",
            Self::Id => "id",
            Self::EmailRouteId => "email_route_id",
            Self::Payload => "payload",
            Self::Attempts => "attempts",
            Self::NextRetryAt => "next_retry_at",
            Self::LastError => "last_error",
            Self::IsExpired => "is_expired",
            Self::CreatedAt => "created_at",
        }
    }
}

pub async fn connect_database(config: &AppConfig) -> Result<DBConnection> {
    install_default_drivers();

    let connection = AnyConnection::connect(&config.database_url)
        .await
        .with_context(|| "Failed to connect to database")?;

    let backend = connection.backend_name();
    info!(backend = backend, "Connected to database");

    Ok(match backend {
        "PostgreSQL" => DBConnection {
            connection,
            wrap_timestamp: true,
            backend: Backend::Postgres,
        },
        "MySQL" => DBConnection {
            connection,
            wrap_timestamp: false,
            backend: Backend::MySql,
        },
        "SQLite" => DBConnection {
            connection,
            wrap_timestamp: false,
            backend: Backend::Sqlite,
        },
        _ => bail!("Unknown backend name: {}", backend),
    })
}

pub async fn initialize_database(mut db: DBConnection, print_only: bool) -> Result<()> {
    if !print_only {
        info!("Creating email_routes table");
    }
    let email_routes = db.backend.build_schema(
        Table::create()
            .table(EmailRoute::Table)
            .if_not_exists()
            .col(
                ColumnDef::new(EmailRoute::Id)
                    .integer()
                    .not_null()
                    .auto_increment()
                    .primary_key(),
            )
            .col(ColumnDef::new(EmailRoute::Domain).string().not_null())
            .col(ColumnDef::new(EmailRoute::User).string().null())
            .col(ColumnDef::new(EmailRoute::Url).string().not_null())
            .col(ColumnDef::new(EmailRoute::SecretToken).string().not_null())
            .col(
                ColumnDef::new(EmailRoute::IsEnabled)
                    .boolean()
                    .not_null()
                    .default(true),
            ),
    );
    if print_only {
        println!("{};", email_routes);
    } else {
        sqlx::query(sqlx::AssertSqlSafe(email_routes))
            .execute(&mut db.connection)
            .await?;
    }

    if !print_only {
        debug!("Creating index idx_route_lookup");
    }
    let email_routes_index = db.backend.build_schema(
        Index::create()
            .name("idx_route_lookup")
            .if_not_exists()
            .table(EmailRoute::Table)
            .col(EmailRoute::Domain)
            .col(EmailRoute::User)
            .col(EmailRoute::IsEnabled),
    );
    if print_only {
        println!("{};", email_routes_index);
    } else {
        sqlx::query(sqlx::AssertSqlSafe(email_routes_index))
            .execute(&mut db.connection)
            .await?;
    }

    if !print_only {
        debug!("Creating index idx_route_enabled_lookup");
    }
    let email_routes_enabled_index = db.backend.build_schema(
        Index::create()
            .name("idx_route_enabled_lookup")
            .if_not_exists()
            .table(EmailRoute::Table)
            .col(EmailRoute::IsEnabled),
    );
    if print_only {
        println!("{};", email_routes_enabled_index);
    } else {
        sqlx::query(sqlx::AssertSqlSafe(email_routes_enabled_index))
            .execute(&mut db.connection)
            .await?;
    }

    if !print_only {
        info!("Creating webhook_queue table");
    }
    let webhook_queue = db.backend.build_schema(
        Table::create()
            .table(WebhookQueue::Table)
            .if_not_exists()
            .col(
                ColumnDef::new(WebhookQueue::Id)
                    .integer()
                    .not_null()
                    .auto_increment()
                    .primary_key(),
            )
            .col(
                ColumnDef::new(WebhookQueue::EmailRouteId)
                    .integer()
                    .not_null(),
            )
            .col(ColumnDef::new(WebhookQueue::Payload).text().not_null())
            .col(
                ColumnDef::new(WebhookQueue::Attempts)
                    .unsigned()
                    .integer()
                    .not_null()
                    .default(0),
            )
            .col(
                ColumnDef::new(WebhookQueue::NextRetryAt)
                    .timestamp_with_time_zone()
                    .not_null()
                    .default(Expr::current_timestamp()),
            )
            .col(ColumnDef::new(WebhookQueue::LastError).text().null())
            .col(
                ColumnDef::new(WebhookQueue::IsExpired)
                    .boolean()
                    .not_null()
                    .default(false),
            )
            .col(
                ColumnDef::new(WebhookQueue::CreatedAt)
                    .timestamp_with_time_zone()
                    .not_null()
                    .default(Expr::current_timestamp()),
            )
            .foreign_key(
                ForeignKey::create()
                    .name("fk_queue_to_route")
                    .from(WebhookQueue::Table, WebhookQueue::EmailRouteId)
                    .to(EmailRoute::Table, EmailRoute::Id),
            ),
    );
    if print_only {
        println!("{};", webhook_queue);
    } else {
        sqlx::query(sqlx::AssertSqlSafe(webhook_queue))
            .execute(&mut db.connection)
            .await?;
    }

    if !print_only {
        debug!("Creating index idx_queue_processing");
    }
    let webhooks_queue_index = db.backend.build_schema(
        Index::create()
            .name("idx_queue_processing")
            .if_not_exists()
            .table(WebhookQueue::Table)
            .col(WebhookQueue::NextRetryAt)
            .col(WebhookQueue::IsExpired),
    );
    if print_only {
        println!("{};", webhooks_queue_index);
    } else {
        sqlx::query(sqlx::AssertSqlSafe(webhooks_queue_index))
            .execute(&mut db.connection)
            .await?;
    }

    Ok(())
}
