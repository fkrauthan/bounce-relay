...
pub async fn execute_ingest(mut db: DBConnection) -> Result<()> {
    ...
    let message = MessageParser::default().parse(&buffer)
        .with_context(|| "Failed to parse email")?;

    let target_address = message.to()
        .and_then(|a| a.first())
        .and_then(|a| a.address.clone())
        .map(|a| a.to_string())
        .unwrap_or("unknown".to_string());

    let domain = target_address.split('@').last().unwrap_or("").trim_matches('>');
    ...
    let routes = sqlx::query_with(&sql, values).fetch_all(&mut db.connection).await
    ...
    let payload = serde_json::json!({
        ...
        "is_permanent": bounce_info.status.chars().nth(0) == Some('5'),
    }).to_string();

    // Insert into webhook queue for delivery
    for route in routes {
        let route_id: i32 = route.try_get(EmailRoute::Id.to_string().as_str())
        ...
        let (sql, values) = Query::insert()
            ...
            .build_any_sqlx(query_builder);
        sqlx::query_with(&sql, values).execute(&mut db.connection).await
...
fn parse_dsn(email: &Message) -> BounceInfo {
    ...
    for part in &email.parts {
        match part.content_type() {
            Some(ct) if ct.c_type == "message" && ct.subtype().unwrap_or("") == "delivery-status" => {
                let text = part.text_contents().unwrap_or("");
                for line in text.lines() {
                    let lower = line.to_lowercase();
                    if lower.starts_with("original-recipient:") {
                        info.recipient = line.split(';').last().unwrap_or("").trim().to_string();
                    } else if lower.starts_with("final-recipient:") && info.recipient.eq("unknown") {
                        info.recipient = line.split(';').last().unwrap_or("").trim().to_string();
                        ...
                        info.status = line.split(':').last().unwrap_or("").trim().to_string();
                    } else if lower.starts_with("action:") {
                        info.action = line.split(':').last().unwrap_or("").trim().to_string();
                    }
                }
            }
            _ => continue,
